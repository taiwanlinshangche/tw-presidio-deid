...
將 `.md` 拖入，辨識疑似個資，再由使用者點擊決定要遮罩哪些內容。

**目前可本機預覽、辨識、人工遮罩、直接編輯，並提供首次啟動與一鍵安裝。專案以 MIT 授權釋出（見 `LICENSE`）。**
...
只需先安裝 Git，不需要先安裝 Node.js、Python、Presidio 或模型，也不需要雲端帳號與 API 金鑰。

1. 在 GitHub 的「Code」複製 clone 指令，下載專案，進入 `tw-presidio-deid` 資料夾。
2. **Apple Silicon Mac（macOS 14 以上）**：雙擊 `start.command`，或在專案終端機執行 `./start.command`。
3. **Windows x64**：雙擊 `start.bat`，或在專案 PowerShell 執行 `.\start.bat`。
4. 瀏覽器自動開啟後，第一次會看到安裝頁，按「安裝缺少的元件」，等待安裝與辨識檢查完成。之後每次啟動都直接進工作台首頁：十張檢查卡片逐張揭露，缺少或失敗的卡片會顯示「安裝」或「重試」按鈕，安裝進度直接寫在那張卡片上；只有網頁建置過期時會自動重建，不必按任何按鈕。

首次啟動若缺少相容 Node.js，啟動視窗會先顯示下載進度；網頁出現後，其餘步驟都在網頁顯示。保留啟動視窗，結束時按 `Control + C`。若瀏覽器沒有自動開啟，複製啟動視窗顯示的本機網址。
...
- 安裝失敗可按「重試」。斷線時保留已完成的快取；損壞的下載不會當成安裝成功。
